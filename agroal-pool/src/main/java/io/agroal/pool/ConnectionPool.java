    ...
    private final class CreateConnectionTask implements Callable<ConnectionHandler> {
        ...
        public ConnectionHandler call() {
            ...
            try {
                ...
                if ( !configuration.lifetime().isZero() ) {
                    handler.setLifetimeFuture( housekeepingExecutor.schedule( new FlushTask( GRACEFUL, handler ), configuration.lifetime().toNanos(), NANOSECONDS ) );
                }
