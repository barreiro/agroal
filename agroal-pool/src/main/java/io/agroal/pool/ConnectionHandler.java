...
public final class ConnectionHandler implements TransactionAware {
    ...
    public void onConnectionWrapperClose(ConnectionWrapper wrapper) throws SQLException {
        // the remove == true ensures there are no concurrent calls to close causing the same handler to be returned multiple times
        if ( enlistedOpenWrappers.remove( wrapper ) && !enlisted ) {
